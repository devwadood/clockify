...
export default async function Invitation({
  ...
  const [invitation] = await getDb()
    ...
    .innerJoin(user, eq(user.id, projectInvitations.inviterId))
    .where(
      and(
  ...
  if (!invitation)
    return (
      <Frame>
        <LockKeyhole className="mx-auto text-[var(--accent)]" />
  ...
  return (
    <Frame>
      ...
      <h1 className="mt-5 text-2xl font-bold">Join {invitation.project}</h1>
      <p className="muted mt-3 text-sm leading-6">
        <b className="text-[var(--text)]">{invitation.inviter}</b> invited{" "}
        <b className="text-[var(--text)]">{invitation.email}</b> to collaborate
        as a {invitation.role}.
      </p>
      <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-4 text-left text-xs">
      ...
      <p className="muted mt-7 text-[10px]">
        Expires{" "}
